...
class ValidateEveryNEpisodes(EventCallback):
    ...
    def __init__(self, every_n_episodes: int, sweep_every_n_episodes: int, validation_log_folder: Path, validation_env, h_network_rl_module: HNetworkRLModule, best_model_save_path: Path):
        super().__init__()
        self.n_episodes = 0
        self.every_n_episodes = every_n_episodes
