...
from rl_env_discrete.battery import RechargeableBatteryDiscrete
from model.H_network.h_network_rl_discrete_act import HNetworkDiscreteRLModule
# from hrl_env_episode import SmartMeterEpisode
from hrl_env_discrete_episode import SmartMeterDiscreteEpisode
from utils import print_log


   

class SmartMeterWorldDiscrete(gym.Env):
    """
    ...
    def __init__(self, smart_meter_data_loader: BaseSmartMeterDataLoader, h_network_rl_module: HNetworkDiscreteRLModule, mode: TrainingMode, log_folder: Path, rb_config: Optional[dict] = None, reward_lambda: float = 0.5, render_mode=None, render_host='127.0.0.1', render_port=50007):
        '''
        Initializes the SmartMeterWorldDiscrete environment.
        ...
        Args:
            ...
            h_network_rl_module (HNetworkDiscreteRLModule): H-network RL module for providing per-step privacy-related signals.
        ...
        '''
        super(SmartMeterWorldDiscrete, self).__init__()
        ...
        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Using data loader: {type(self.smart_meter_data_loader).__name__}")
        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Curriculum enabled: {self.curriculum_info.get('curriculum_enabled', False)}")
        if self.curriculum_info.get('curriculum_enabled', False):
            print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Episode lengths available: {list(self.curriculum_info.get('episodes_by_length', {}).keys())} days")
            print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Total episodes: {self.curriculum_info.get('total_episodes', 0)}")

        # initialize an episode
        self.selected_idx = 0
        self.episode = SmartMeterDiscreteEpisode(self.smart_meter_data_loader.get_aggregate_load_segment(self.selected_idx))  # start with the first DataFrame
        self.episode_info_list = []

        # Initialize battery state
        # init_soc is the constant initial state of charge (SoC) of the battery for each episode beginning
        self.init_soc = Decimal(str(0.15)) if rb_config is None else rb_config.get('init_soc', Decimal(str(0.5)))  # default to 50% if not specified
        self.battery = RechargeableBatteryDiscrete(**rb_config) if rb_config else RechargeableBatteryDiscrete(
                capacity=Decimal(str(8.0)),  # kWh
                max_charging_rate=Decimal(str(4.0)),  # kW
                efficiency=Decimal(str(1.0)),  # 100% efficiency
        ...
        assert int(self.battery.max_charging_rate % Decimal(str(0.05))) == 0, "Max charging/discharging rate must be a multiple of the step size of charging/discharging action."
        ...
        if self.render_mode == "human":
            ...
            print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}]] Render mode set to '{self.render_mode}'. Render server at {self.render_host}:{self.render_port}. render_connected: {self.render_connected}. render_client_socket: {self.render_client_socket}")
    ...
    def _init_render_client(self):
        ...
        try:
            ...
            print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Could not connect to render server: {e}")
            self.render_connected = False
    ...
    def reset(self, episode_idx: int = None, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.episode = SmartMeterDiscreteEpisode(self.smart_meter_data_loader.get_aggregate_load_segment(self.selected_idx))  # Reset with a new episode
        self.episode_info_list = []
        ...
        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Resetting environment with episode {self.selected_idx}. Episode info: {self.episode.get_episode_info()}")
        if self.curriculum_info.get('curriculum_enabled', False):
            print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Episode length: {episode_length_days} day(s), Training timestep: {self.training_timestep}")
    ...
    def step(self, action):
        ...
        # the action is an integer in range [0, 2N]
        _action = action
        ...
        self.episode.df.iat[current_step, self.episode.df.columns.get_loc('battery_soc')] = float(self.battery.get_normalized_state_of_charge())

        # Apply the action to the battery
        power_charged_discharged = self.battery.charge_discharge(power_kw, Decimal(int(duration)), Decimal(int(y_t)))
        ...
        f_signal, f_signal_additional_info = self._f_signal(
            y_t=y_t,
            z_t=z_t,
            y_t_plus_1=self.episode.df.iloc[current_step + 1]['aggregate_logit']
        )
        ...
        if terminated:
            ...
            print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Episode finished. Sum of rewards: {episode_sum}. Mean of rewards: {episode_reward_stats['mean']}. Std of rewards: {episode_reward_stats['std']}")
    ...
    def _get_info(self, obs, power_kw=None, power_charged_discharged=None, reward=None, f_signal=None, g_signal=None, f_signal_additional_info=None):
        ...
        info_dict = {
            ...
            "battery_soc (kWh)": float(obs["battery_soc"][0] * float(self.battery.capacity)),  # convert normalized SoC to kWh
    ...
    def set_training_timestep(self, timestep: int):
        ...
        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Training timestep set to {timestep}")
    ...
    def set_log_folder(self, log_folder: Path):
        ...
        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Log folder set to {self.log_folder}")
    ...
    def _g_signal(self, s_t_datetime:datetime, s_t_plus_1_datetime:datetime, power_kw: float) -> float:
        ...
        return _get_weighted_electricity_cost(s_t_datetime, s_t_plus_1_datetime) * abs(float(power_kw)) + standing_charge_per_day * ((s_t_plus_1_datetime - s_t_datetime).total_seconds() / (24 * 60 * 60))  # convert seconds to days
    ...
    def _f_signal(self, y_t: int, z_t: int, y_t_plus_1: int) -> float:
        ...
        Args:
            y_t (int): User's load at time t.
            z_t (int): Load received from the grid at time t.
            y_t_plus_1 (int): User's load at time t+1, but mapped to the quantized load logit [0, 100]
        Returns:
        ...
        self.h_network_episode_inference_buffer.append(((y_t, int(z_t)), y_t_plus_1))
        ...
        # for discrete action, the target will be array with shape (n_samples, 1),
        # where the feature is quantized load value mapped from [0, 5kW] to [0, 100]
        # this is already performed at initialization of the episode
    ...
    def save_episodes_rewards(self, folder_path: Path):
        ...
        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Episodes rewards saved to {file_path}")
    ...
    def save_env_config(self, log_folder: Path):
        ...
        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Environment config saved to {env_config_path}")
    ...
    def _send_json_message(self, message: dict):
        ...
        if self.render_connected and self.render_client_socket:
            try:
                ...
                print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Failed to send message to render server: {e}")
    ...
    def close(self):
        ...
        if self.render_client_socket:
            ...
            try:
                ...
                print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Error closing render client socket: {e}")
            self.render_client_socket = None
            self.render_connected = False

        print_log(f"[SmartMeterWorldDiscrete {str(self.mode).capitalize()}] Environment closed.")
