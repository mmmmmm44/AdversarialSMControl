...
class HNetworkRLModule:
    ...
    def compute_f_signal(self, h_network_input, h_network_target):
        ...
        return signal_output.item(), \
                {'predicted_mean': float(mean_masked[-1].item()), 
                 'predicted_log_var': float(log_var_masked[-1].item()) if self.h_network_type.value == HNetworkType.H_NETWORK2.value else 0,   # for H-Network, we fixed the variance to 1.0 -> log_var is 0
                 'target': float(_target_masked[-1].item())
                }
